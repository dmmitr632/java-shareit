...
public interface ItemRepository extends JpaRepository<Item, Integer> {
    ...
    @Query(value = "SELECT i.item_id as id, i.name as name, i.description as description, i.available as " +
            ...
            "from items as i left join bookings as lastBooking on i.item_id = lastBooking.item_id " +
            "left join bookings as nextBooking on i.item_id = nextBooking.item_id " +
            "where (lastBooking.booking_id is null or nextBooking.booking_id is null) and (i" +
            ".owner_id = :userId) " +
    ...
    @Query(value = "SELECT i.item_id as id, i.name as name, i.description as description, i.available as " +
            ...
            "nextBooking.booking_id as nextBookingId, nextBooking.booker_id as nextBookingBookerId, " +
            "nextBooking.start_time as nextStart, nextBooking.end_time as nextEnd, " +
            "i.owner_id as ownerId " + "from items as i " +
